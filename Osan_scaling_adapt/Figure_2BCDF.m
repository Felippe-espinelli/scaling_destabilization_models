...
for mismatch_neurons = 0:max_mismatch  
...
mean_reexposure_control = zeros(max_mismatch+1, 1);
mean_reexposure_aniso = zeros(max_mismatch+1, 1);
for mismatch_neurons = 0:max_mismatch  
...
SEM_reexposure_control = zeros(max_mismatch+1,1);
SEM_reexposure_aniso = zeros(max_mismatch+1,1);
for mismatch_neurons = 0:max_mismatch  
...
% FIGURE - EFFECT OF ANISOMYCIN IN A SHORT REEXPOSURE - Mismatch 1
fig2 = figure(2);

% Plot
x = [1 2 3 4 5];
y = [mean_training_control mean_training_aniso 0 mean_reexposure_control(2) mean_reexposure_aniso(2)];
...
errorbar(4, y(4), SEM_reexposure_control(2,:));
errorbar(5, y(5), SEM_reexposure_aniso(2,:));
hold off
...
% FIGURE - EFFECT OF ANISOMYCIN IN A MEDIUM REEXPOSURE - Mismatch 7
fig3 = figure(3);

% Plot
x = [1 2 3 4 5];
y = [mean_training_control mean_training_aniso 0 mean_reexposure_control(7) mean_reexposure_aniso(7)];
b2 = bar(x, y, 'FaceColor','flat');
b2.CData(1,:) = [0.6679 0.4062 0.3398];
b2.CData(2,:) = [0.4453 0.5742 0.7929];
b2.CData(4,:) = [0.6679 0.4062 0.3398];
...
errorbar(1, y(1), SEM_training_control);
errorbar(2, y(2), SEM_training_aniso);
errorbar(4, y(4), SEM_reexposure_control(7,:));
errorbar(5, y(5), SEM_reexposure_aniso(7,:));
hold off
ylabel('Freezing percentage', 'fontsize', 24);
ylim([0 100]);
yticks([0 50 100]);
...
% FIGURE - EFFECT OF ANISOMYCIN IN A LONG REEXPOSURE - Mismatch 10
fig4 = figure(4);
% Plot
x = [1 2 3 4 5];
y = [mean_training_control mean_training_aniso 0 mean_reexposure_control(11) mean_reexposure_aniso(11)];
b2 = bar(x, y, 'FaceColor','flat');
b2.CData(1,:) = [0.6679 0.4062 0.3398];
b2.CData(2,:) = [0.4453 0.5742 0.7929];
b2.CData(4,:) = [0.6679 0.4062 0.3398];
...
errorbar(1, y(1), SEM_training_control);
errorbar(2, y(2), SEM_training_aniso);
errorbar(4, y(4), SEM_reexposure_control(11,:));
errorbar(5, y(5), SEM_reexposure_aniso(11,:));
hold off
ylabel('Freezing percentage', 'fontsize', 24);
ylim([0 100]);
yticks([0 50 100]);
...
% FIGURE - SUMMARY OF THE EFFECTS OF ANISOMYCIN
fig5 = figure(5);

% Plot
e_sum_control = errorbar(0:1:max_mismatch,...
    mean_reexposure_control,...
    SEM_reexposure_control...
    , 'LineWidth', 1);
e_sum_control.Color = [0.6679 0.4062 0.3398];
hold on;
e_sum_aniso = errorbar(0:1:max_mismatch,...
    mean_reexposure_aniso,...
    SEM_reexposure_aniso...
    , 'LineWidth', 1);
e_sum_aniso.Color = [0.4453 0.5742 0.7929];
