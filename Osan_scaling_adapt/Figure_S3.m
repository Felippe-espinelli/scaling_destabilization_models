...
S_max_value_training = 2;
Steps = 0.1;
S_values_training = 0:Steps:S_max_value_training;
...
weight_second_session_control = zeros(100,100,simulation_quantity, length(S_values_training));
Ratio_session_control_training_retr = zeros(100,1,simulation_quantity, length(S_values_training));
Activity_retrieval_control_training = zeros(1,100,simulation_quantity, length(S_values_training));

weight_second_session_aniso = zeros(100,100,simulation_quantity, length(S_values_training));
Ratio_session_aniso_training_retr = zeros(100,1,simulation_quantity, length(S_values_training));
Activity_retrieval_aniso_training = zeros(1,100,simulation_quantity, length(S_values_training));

for TTTT = 1:length(S_values_training)
    for LLLL = 1:simulation_quantity
        % CONTROL GROUP
        ...
        weight_second_session_control(:,:,LLLL, TTTT) = Weight_rules_controlled_noise_V2(weight_update, Learning_rounds_training, t_initial, t_final, nr_neurons_h,...
            patterns_h, Ix, decay,  S_values_training(TTTT), ratio_HLP_SC, saturation, Learning_noise, Learning_noise_state, general_SC_on_off, vT);
...
heatmap_control.YLabel = 'S value in training learning';
heatmap_control.Colormap = jet;
heatmap_control.YDisplayLabels = num2cell(0:Steps:S_max_value_training);
...
figure;
set(gcf,'renderer','Painters');
heatmap_aniso = heatmap(mean_reexposure_aniso, 'CellLabelColor','none');
heatmap_aniso.Title = 'Anisomycin';
heatmap_aniso.XLabel = 'Reexposure time (t)';
heatmap_aniso.YLabel = 'S value in training learning';
heatmap_aniso.Colormap = jet;
heatmap_aniso.YDisplayLabels = num2cell(0:Steps:S_max_value_training);
