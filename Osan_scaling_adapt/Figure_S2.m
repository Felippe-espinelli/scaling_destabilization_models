...
%%% REACTIVATION
Scaling_max_value_reexposure = 4;
Scaling_steps = 0.2;
Scaling_values_reexposure = 0:Scaling_steps:Scaling_max_value_reexposure;
S_reexposure = 0.8;
S_aniso = 0;
...
weight_first_session_aniso = zeros(100,100,simulation_quantity);
...
weight_second_session_control = zeros(100,100,simulation_quantity, length(Scaling_values_reexposure));
Ratio_session_control_training_retr = zeros(100,1,simulation_quantity, length(Scaling_values_reexposure));
Activity_retrieval_control_training = zeros(1,100,simulation_quantity, length(Scaling_values_reexposure));

weight_second_session_aniso = zeros(100,100,simulation_quantity, length(Scaling_values_reexposure));
Ratio_session_aniso_training_retr = zeros(100,1,simulation_quantity, length(Scaling_values_reexposure));
Activity_retrieval_aniso_training = zeros(1,100,simulation_quantity, length(Scaling_values_reexposure));

for TTTT = 1:length(Scaling_values_reexposure)
    for LLLL = 1:simulation_quantity
        % CONTROL GROUP
        ...
        weight_update = weight_first_session_aniso(:,:,LLLL);
        weight_second_session_aniso(:,:,LLLL, TTTT) = Weight_rules_controlled_noise_V2(weight_update, Learning_rounds_training, t_initial, t_final, nr_neurons_h,...
            patterns_h, Ix, decay, S_training, ratio_HLP_SC, saturation, Learning_noise, Learning_noise_state, general_SC_on_off, vT);
        
        weight_update = weight_second_session_aniso(:,:,LLLL, TTTT);
        Ix = cue_factor * retrieval_stimulus;
        retrieval_controlled_noise;
        ...
        Ratio_session_aniso_training_retr(:,:,LLLL, TTTT) = (session_aniso_train_cue_shock ./ session_aniso_train_cue_Non_shock_aniso);
        Activity_retrieval_aniso_training(:,:,LLLL, TTTT) = mean(mean_act_all_neurons);
...
Ratio_session_control_extinction_cue = zeros(100 , 1, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
weight_extinction_control = zeros(nr_neurons_h, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
React_control_within_session_activity = zeros(1, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
Activity_ret_all_neurons_control = zeros(1, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));

Ratio_session_aniso_extinction_cue = zeros(100 , 1, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
weight_extinction_aniso = zeros(nr_neurons_h, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
React_aniso_within_session_activity = zeros(1, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
Activity_ret_all_neurons_aniso = zeros(1, nr_neurons_h, Quant_ext_sessions, simulation_quantity, max_mismatch+1, length(Scaling_values_reexposure));
...
%             Ix(Non_shock_neurons(mismatch_neurons_to_add)) = Input_reexposure_factor;
            ...
            for iii = 1:Quant_ext_sessions
                [weight_extinction_control(:, :, iii, LLLL, mismatch_neurons+1, TTTT), React_control_within_session_activity(:, :, iii, LLLL, mismatch_neurons+1, TTTT)] = Weight_rules_post_react_controlled_noise_V2(weight_update, nr_learning_rounds,...
                    t_initial, t_final, nr_neurons_h, patterns_h, Ix, decay, S_reexposure, Scaling_values_reexposure(TTTT), saturation, Reexposure_noise, Reexposure_noise_state, general_SC_on_off, vT);
...
%             Ix(Non_shock_neurons(mismatch_neurons_to_add)) = Input_reexposure_factor;
            ...
            %  Reactivation Session
            for iii = 1:Quant_ext_sessions
                [weight_extinction_aniso(:, :, iii, LLLL, mismatch_neurons+1, TTTT), React_aniso_within_session_activity(:, :, iii, LLLL, mismatch_neurons+1, TTTT)] = Weight_rules_post_react_controlled_noise_V2(weight_update, nr_learning_rounds,...
                    t_initial, t_final, nr_neurons_h, patterns_h, Ix, decay, S_aniso, Scaling_values_reexposure(TTTT), saturation, Reexposure_noise, Reexposure_noise_state, general_SC_on_off, vT);
                weight_update = weight_extinction_aniso(:, :, iii, LLLL, mismatch_neurons+1, TTTT);
                %     first_extinction_no_shock_session_activity = first_extinction_no_shock_session_activity(end, :);
            end
            
            if Reactivation_choice_within_retrieval == 0
                for iiii = 1:Quant_ext_sessions
                    weight_update = weight_extinction_aniso(:,:,iiii, LLLL, mismatch_neurons+1, TTTT);
                    Ix = cue_factor * retrieval_stimulus;
                    retrieval_controlled_noise;
                    old_reexposure_Aniso(mismatch_neurons+1, :, TTTT) = ode23_attractors_cortex;
                    session_ANISO_react_cue_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT) = shock_neuron_activity;
                    session_ANISO_react_cue_Non_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT) = Non_shock_neuron_activity;
                    % To adjust the ratio
                    session_ANISO_react_cue_Non_shock_and_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT) = abs(session_ANISO_react_cue_Non_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT)...
                        ) + abs(session_ANISO_react_cue_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT));
                    Ratio_session_aniso_extinction_cue(:,:,iiii, LLLL, mismatch_neurons+1, TTTT) = abs(session_ANISO_react_cue_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT)...
                        ) ./ session_ANISO_react_cue_Non_shock_and_shock(:,:,iiii, LLLL, mismatch_neurons+1, TTTT);
                    Activity_ret_all_neurons_aniso(:,:,iiii, LLLL, mismatch_neurons+1, TTTT) = mean(mean_act_all_neurons);
...
p_reexposure_aniso(:,:,:) = 100*(rand(size(old_reexposure_control)) < freezing_probabilities(old_reexposure_Aniso(:,:,:)));

% Mean calculation
mean_reexposure_control = mean(p_reexposure_control, 2);
mean_reexposure_aniso = mean(p_reexposure_aniso, 2);

mean_reexposure_control = squeeze(mean_reexposure_control);
mean_reexposure_control = mean_reexposure_control';
mean_reexposure_aniso = squeeze(mean_reexposure_aniso);
mean_reexposure_aniso = mean_reexposure_aniso';

%% FIGURE - HEATMAP FOR CONTROL GROUP
heatmap_control = heatmap(mean_reexposure_control);
heatmap_control.Title = 'Vehicle';
heatmap_control.XLabel = 'Reexposure time (t)';
heatmap_control.YLabel = 'Scaling value in reexposure';
heatmap_control.Colormap = jet;
heatmap_control.YDisplayLabels = num2cell(0:Scaling_steps:Scaling_max_value_reexposure);
heatmap_control.XDisplayLabels = num2cell(0:1:10);
colorbar;
caxis([0, 100]);

figure;
heatmap_aniso = heatmap(mean_reexposure_aniso);
heatmap_aniso.Title = 'Anisomycin';
heatmap_aniso.XLabel = 'Reexposure time (t)';
heatmap_aniso.YLabel = 'Scaling value in reexposure';
heatmap_aniso.Colormap = jet;
heatmap_aniso.YDisplayLabels = num2cell(0:Scaling_steps:Scaling_max_value_reexposure);
heatmap_aniso.XDisplayLabels = num2cell(0:1:10);
colorbar;
caxis([0, 100]);
